...
describe Rnp.instance_method(:generate_rsa),
         skip: !LibRnp::HAVE_RNP_GENERATE_KEY_RSA do
  context "when generating primary only" do
    let(:key) do
      @rnp = Rnp.new
      @rnp.generate_rsa(userid: "test1", bits: 1024, password: nil)
    end
    ...
    context "when a password is provided" do
      let(:key) do
        @rnp = Rnp.new
        @rnp.generate_rsa(userid: "test1", bits: 1024, password: "pass")
      end
  ...
  context "when generating a primary and sub" do
    let(:key) do
      @rnp = Rnp.new
      @rnp.generate_rsa(userid: "test1", bits: 1024, subbits: 1024,
                        password: nil)
    end
