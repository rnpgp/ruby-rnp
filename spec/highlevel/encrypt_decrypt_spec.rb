...
describe 'encrypting and decrypting' do
  ...
  context 'with pubkey_no_pass.asc and seckey_no_pass.asc' do
    let(:pubkeys) { RNP::load_keys(File.read('spec/keys/pubkey_no_pass.asc'), true) }
    let(:seckeys) { RNP::load_keys(File.read('spec/keys/seckey_no_pass.asc'), true) }
  ...
  context 'with pubkey_sign_only.asc and seckey_sign_only.asc' do
    let(:pubkeys) { RNP::load_keys(File.read('spec/keys/pubkey_sign_only.asc'), true) }
    let(:seckeys) { RNP::load_keys(File.read('spec/keys/seckey_sign_only.asc'), true) }
