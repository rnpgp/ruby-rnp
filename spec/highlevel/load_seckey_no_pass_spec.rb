...
describe RNP.method(:load_keys) do
  ...
  context 'when loading seckey_no_pass.asc' do
    let(:keys) { RNP::load_keys(File.read('spec/keys/seckey_no_pass.asc'), true) }
    ...
    context 'keys[0]' do
      ...
      it 'has expected properties' do
        ...
        expect(key.string_to_key_usage).to eql RNP::StringToKeyUsage::NONE
        expect(key.string_to_key_specifier).to eql RNP::StringToKeySpecifier::SIMPLE
        expect(key.symmetric_key_algorithm).to eql RNP::SymmetricKeyAlgorithm::PLAINTEXT
