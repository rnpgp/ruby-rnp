...
describe RNP.method(:load_keys) do
  ...
  context 'when loading seckey_sign_only.asc' do
    let(:keys) { RNP::load_keys(File.read('spec/keys/seckey_sign_only.asc'), true) }
    ...
    context 'keys[0]' do
      ...
      it 'has expected properties' do
        ...
        expect(key.string_to_key_specifier).to eql RNP::StringToKeySpecifier::ITERATED_AND_SALTED
