...
describe Rnp.instance_method(:generate),
         skip: !LibRnp::HAVE_RNP_GENERATE_KEY_EX do
  context "when generating primary only" do
    let(:key) do
      @rnp = Rnp.new
      @rnp.generate(type: "RSA", userid: "test1", bits: 1024, password: nil)
    end
    ...
    context "when a password is provided" do
      let(:key) do
        @rnp = Rnp.new
        @rnp.generate(type: "RSA", userid: "test1", bits: 1024,
                      password: "pass")
      end
