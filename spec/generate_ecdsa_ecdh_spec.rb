...
describe Rnp.instance_method(:generate_ecdsa_ecdh),
         skip: !LibRnp::HAVE_RNP_GENERATE_KEY_EC do
  let(:key) do
    @rnp = Rnp.new
    @rnp.generate_ecdsa_ecdh(userid: "test1", curve: "secp256k1", password: nil)
  end

  let(:subkey) { key.subkeys[0] }

  it "has the correct type" do
    expect(key.type).to eql "ECDSA"
  ...
  it "has the correct curve" do
    expect(key.curve).to eql "secp256k1"
    expect(subkey.curve).to eql "secp256k1"
  end
  ...
  context "when a password is provided" do
    let(:key) do
      @rnp = Rnp.new
      @rnp.generate_ecdsa_ecdh(userid: "test1", curve: "secp256k1",
                               password: "pass")
    end
