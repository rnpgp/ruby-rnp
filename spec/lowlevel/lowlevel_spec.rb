...
describe 'rsa key generation' do
  before(:all) {
    @key = LibRNP::pgp_rsa_new_selfsign_key(1024, 65537, 'John Doe', 'SHA1', 'cast5')
    expect(@key.null?).to be false
    expect(
      LibRNP::pgp_add_userid(@key, 'Joe Doe')
    ).to_not eql nil
    expect(
      LibRNP::pgp_add_selfsigned_userid(@key, 'Jane Doe')
    ).to eql 1
  }
  after(:all) {
    LibRNP::pgp_keydata_free(@key)
  }

  it 'has correct userids' do
    expect(LibRNP::dynarray_count(@key, 'uid')).to eql 3
    expect(LibRNP::dynarray_get_item(@key, 'uid', :string, 0)).to eql 'John Doe'
    expect(LibRNP::dynarray_get_item(@key, 'uid', :string, 1)).to eql 'Joe Doe'
    expect(LibRNP::dynarray_get_item(@key, 'uid', :string, 2)).to eql 'Jane Doe'
  end
  ...
  it 'has valid packets' do
    expect(
      LibRNP::dynarray_count(@key, 'packet')
    ).to eql 2
    (0..LibRNP::dynarray_count(@key, 'packet') - 1).each {|n|
      ...
      packet = LibRNP::dynarray_get_item(@key, 'packet', LibRNP::PGPSubPacket, n)
  ...
  it 'seckey pubkey has expected properties' do
    ...
    expect(
      LibRNP::bn2hex(pubkey[:key][:rsa][:e]).hex
    ).to eql 65537
  end
  ...
  it 'seckey has valid rsa mpi' do
    ...
    LibRNP::bn2hex(rsaseckey[:d])
    LibRNP::bn2hex(rsaseckey[:p])
    LibRNP::bn2hex(rsaseckey[:q])
    LibRNP::bn2hex(rsaseckey[:u])
  end
